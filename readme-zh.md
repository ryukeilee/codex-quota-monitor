...
Codex Monitor 是一款隐私优先、完全本地运行的 macOS 菜单栏应用，用于以低干扰方式追踪 Codex 使用额度与状态，并提供接近原生系统工具的使用体验。
...
- 基于 Electron 的本地桌面应用
- 可直接双击启动的 macOS `.app` 独立客户端
- 优先读取本机 Codex app-server 的实时额度数据，必要时回退到本地快照
- 菜单栏顶部优先显示周额度剩余百分比
- 菜单内明确展示最近刷新时间和重置时间
- 主窗口同步展示周重置时间、心流预测和开发状态
- 紧凑的主窗口统计面板
- 支持开机自启动
- 支持纯菜单栏模式并隐藏 Dock
...
构建完成后，直接在 Finder 中打开 `dist/Codex Monitor.app` 即可启动，不会再弹出终端窗口。

`run.command` 仍保留为开发辅助入口，但日常打开建议使用打包后的 `.app`。
...
- 菜单栏标题以纯百分比形式显示当前周额度剩余，更贴近原生 macOS 菜单栏工具的长期监控体验
- 菜单中使用分区标签显示周额度、重置时间、5 小时窗口和最近刷新
- `立即刷新` 被单独隔离，通知和开机自启动被下移到设置区，减少误触
- 下方设置区可打开 Dashboard 并调整菜单栏相关偏好
...
- 任意一次唤醒刷新成功后会取消后续重试
- 解锁桌面会触发一次强制刷新
- 长时间睡眠后，如果 live 读取仍然回退到本地 thread 数据，会重新锚定当前 5 小时窗口，避免旧的本地状态污染当前额度
